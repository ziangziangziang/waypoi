...
async function start(): Promise<void> {
  ...
  try {
    ...
    console.error(`[${APP_NAME}] Failed to rebuild smart virtual model: ${(error as Error).message}`);
  ...
  try {
    ...
    console.log(
      `[${APP_NAME}] Providers: total=${providers.length}, enabled=${enabledProviders}, virtualModels=${virtualModels.length}`
    );
